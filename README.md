...
- 提供库存、销售订单、发票账龄、应收账龄、超期风险合并、采购订单和本人费用报销单查询
...
只返回本人数据的费用报销和审批进度模块默认向所有已登录用户开放，管理员仍可通过名单进一步限制入口。查询台按模块分别保存当前页面会话内的查询结果；切换到未查询模块时不显示其他模块的数据，切回后恢复该模块上一次的结果和排序。

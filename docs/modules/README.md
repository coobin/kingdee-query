...
| [审批进度](workflow-progress.md) | `workflow_progress` | 单据流程对象 | 查询指定单据的当前审批节点和历史 |
...
- 页面使用结构化条件；普通标准查询不接受任意 SQL、金蝶过滤表达式、任意 `FormId`、返回字段或 WebAPI 方法作为用户输入。审批进度的 `formId` 只作为已配置只读接口的业务参数。
